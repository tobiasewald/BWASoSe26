...
# LLM configurations stored in memory, overriding environment if set by user
llm_config = {
    "LLM_MODE": os.environ.get("LLM_MODE", "simulator"),
...
def process_invoice(invoice: InvoiceRequest):
    ...
    # Reset DB on new invoice to keep demo isolated if desired, or keep it persistent.
    # We will let it be persistent so the database updates accumulate.
    ...
    try:
        # Run graph. It will execute until completion or hit interrupt_before manager_approval_node
...
def approve_invoice(req: ApprovalRequest):
    ...
    try:
        # Inject manager decision into state
...
@app.get("/api/db")
def get_db_state():
    """Returns the current state of tables in SQLite database (for visual viewer)."""
...
# Serve static files (css, js) directly if they are requested
@app.get("/styles.css")
def get_css():
